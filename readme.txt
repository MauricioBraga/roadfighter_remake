...
A couple of behavioural notes:
 - Key bindings are now stored as SDL scancodes (physical keys) instead of
   ...
   A pre-existing RoadFighter.cfg from the old SDL 1.2 build will be
   ignored/regenerated with defaults the first time you run this build.
 - The car engine/skid "pitch bend" sound effect now uses SDL3_mixer's
   native per-track frequency ratio and stereo gain controls instead of
   manually resampling raw PCM buffers every frame.
...
Programming: Santi Onta��n
Graphics: Miikka Poikela
Music/SFX: Jorrith Schaap
Beta Testing: Jason Eames, Miikka Poikela, Jorrith Schaap, Santi Onta��n
