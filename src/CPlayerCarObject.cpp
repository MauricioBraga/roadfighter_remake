	...
	if (state!=4 && y>0 && fuel > 0) {
		if (game->S_carengine!=0) {
			if ((sound_timmer&0x07)==0) {
				...
				/* Split-screen 2-player mode: pan each player's engine hard to
				   their own side, like the original manual L/R buffer mixing did. */
				float pan_l=1.0F, pan_r=1.0F;
				if (game->focusing_objects.Length()!=1) {
					if (game->focusing_objects[0]==this) { pan_l=0.0F; pan_r=1.0F; }
						else { pan_l=1.0F; pan_r=0.0F; }
				} /* if */

				if (enginesound_channel==-1) {
					enginesound_channel=Sound_play_loop_ch(game->S_carengine,-1,-1);
				} /* if */
				...
				if (state==5 || state==6) {
					...
					skidsound_channel=Sound_play_loop_ch(game->S_carskid,skidsound_channel,0);
					Sound_set_channel_frequency(skidsound_channel,f);
				} else {
